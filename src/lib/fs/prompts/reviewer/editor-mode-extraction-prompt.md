...
- **Rule 1 (Name Uniqueness):** Flag reused names. New characters **must not** use names from this list of known characters: `{knownCharacterNameList}`. Also flag the following naming violations:
  ...
  - **Secret or unnamed characters:** If a **new** recurring character, including the player character, is introduced without an explicit in-scene name, the narrator must have privately assigned them a name. Flag if no such name is resolvable from the `### Player Aliases` and `### Other Character Aliases` Markdown section.
